...
EventRouter.get("/all/:id", async (req, res) => {
  const { id } = req.params;
  try {
    let events = await EventModel.find({ organiser: { $ne: id } }).populate(
      "playingMembers"
    );
...
EventRouter.get("/own/:id", async (req, res) => {
  const { id } = req.params;
  try {
    let events = await EventModel.find({ organiser: { $eq: id } }).populate("receivedRequests");
    res.status(200).send(events);
  } catch (error) {
    console.log(error);
    res.status(400).send(error);
...
EventRouter.get("/single/:id", async (req, res) => {
  const { id } = req.params;
  try {
    const event = await EventModel.findById(id).populate("receivedRequests");
    res.status(200).send(event);
  } catch (error) {
    console.log(error);
    res.status(400).send(error);
