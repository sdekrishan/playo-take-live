...
const ProfileEventComponent = ({event}) => {
    ...
    const acceptRequestFun = (id) => {
      axios
        .patch(`http://localhost:8080/event/accept/${id}`, {
    ...
    const cancelRequestFun = (id) => {
      axios
        .patch(`http://localhost:8080/event/cancel/${id}`, {
          eventId: singleEvent._id,
        },{
          headers:{
            'authorization':token
        ...
        {event.otherReq ? (
          ...
          <Text className="bighead">No Other Requirements</Text>
            ...
            <Box>
              {
                <Box>
                  <Text className="bighead single_event_inner_white" color='white' >Player's Request</Text>
                  <Box>
                    {event.receivedRequests && event.receivedRequests.length > 0 ?
                      ...
                      }) : <Text className='bighead' color='white'>No one has Applied yet !</Text>
                    }
                  </Box>
                </Box>
              }
